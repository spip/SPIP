...
function calculer_visites($t)
{
	...
	foreach(array_keys($visites) as $date)
		if ($visites[$date]) {
			...
			if (!Sql::countsel('spip_visites', "date='$date'"))
				Sql::insertq('spip_visites',
					array('date' => $date, 'visites' => $visites[$date]));
			else 
				Sql::update('spip_visites', array('visites' => "visites+".intval($visites[$date])), "date='$date'");
			...
			if ($visites_a[$date]) {
				...
				foreach($visites_a[$date] as $id_article => $n) {
					if (!Sql::countsel('spip_visites_articles',
						 "id_article=$id_article AND date='$date'")){
						Sql::insertq('spip_visites_articles',
				...
				foreach ($ar as $n => $liste) {
					$tous = Sql::in('id_article', $liste);
					Sql::update('spip_visites_articles',
			...
			if ($referers[$date]) {
				...
				foreach ($referers[$date] as $referer => $num) {
					$referer_md5 = Sql::hex(substr(md5($referer), 0, 15));
					$referer = substr($referer,0,$n);
					if (!Sql::countsel('spip_referers', "referer_md5=$referer_md5"))
						Sql::insertq('spip_referers',
							array('visites' => $num,
							      'visites_jour' => $num,
							      'visites_veille' => $num,
							      'date' => $date,
			...
			// attention on appelle Sql::in en mode texte et pas array
			// pour ne pas passer Sql::quote() sur les '0x1234' de referer_md5, cf #849
				foreach ($ar as $num => $liste) {
					Sql::update('spip_referers', array('visites' => "visites+$num", 'visites_jour' => "visites_jour+$num"), Sql::in('referer_md5',join(', ', $liste)));
				}
			}
			...
			if ($referers_a[$date]) {
				...
				foreach ($referers_a[$date] as $id_article => $referers)
					foreach ($referers as $referer => $num) {
						...
						if (!Sql::countsel('spip_referers_articles', $prim))
							Sql::insertq('spip_referers_articles',
							     array('visites' => $num,
				...
				// ajouter les visites
				foreach ($ar as $num => $liste) {
					Sql::update('spip_referers_articles', array('visites' => "visites+$num"), join(" OR ", $liste));
					## Ajouter un JOIN sur le statut de l'article ?
				}
			}
		}
