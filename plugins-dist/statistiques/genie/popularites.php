...
function genie_popularites_dist($t) {
	...
	Sql::update('spip_articles', array('maj'=>'maj', 'popularite' => "popularite * $a"), 'popularite>1');

	// enregistrer les metas...
	$row = Sql::fetsel('MAX(popularite) AS max, SUM(popularite) AS tot', "spip_articles");
	...
	if (($d = $GLOBALS['meta']['date_statistiques']) != $aujourdhui) {
		...
		if (strncmp($GLOBALS['connexions'][0]['type'],'sqlite',6)==0)
		  ...
		  // ATTENTION : peut poser probleme cf http://core.spip.org/issues/2505
			Sql::alter("TABLE spip_referers DROP visites_veille,
