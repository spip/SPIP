...
function inc_lien_dist($lien, $texte='', $class='', $title='', $hlang='', $rel='', $connect='', $env=array()) {
	...
	AND $match = typer_raccourci($lien)) { 
		...
		if ($desc
			...
			if ($row=Sql::fetsel('*', $table_objet_sql, "$id_table_objet=".intval($id))
				AND isset($row['id_trad'])
				AND isset($row['lang'])
				AND $id_dest = Sql::getfetsel($id_table_objet, $table_objet_sql,"id_trad=".intval($row['id_trad'])." AND lang=" . Sql::quote($hlang))
