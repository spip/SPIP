...
function action_acceder_document_dist() {
	...
	OR preg_match(',^\w+://,', $f)) {
		...
		$where = "documents.fichier=".Sql::quote(set_spip_doc($file))
		. ($arg ? " AND documents.id_document=".intval($arg): '');

		$doc = Sql::fetsel("D.id_document, D.titre, D.fichier, T.mime_type, T.inclus, D.extension", "spip_documents AS D LEFT JOIN spip_types_documents AS T ON D.extension=T.extension",$where);
		if (!$doc) {
			$status = 404;
		} else {
