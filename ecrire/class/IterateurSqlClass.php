...
class IterateurSQL implements Iterator
{
	...
	**/
	...
	{
		if (!Sql::seek($this->sqlresult, $n, $this->command['connect'], $continue)) {
		...
		$this->row = Sql::fetch($this->sqlresult, $this->command['connect']);
	...
	public function free()
	{
		...
		$a = Sql::free($this->sqlresult, $this->command['connect']);
		$this->sqlresult = null;
		return $a;
	}
		...
		if (is_null($this->total)) {
			if (!$this->sqlresult) {
				...
				if (in_array('count(*)', $this->command['select'])) {
					...
					$this->total = Sql::count($this->sqlresult, $this->command['connect']);
			}
		}
		return $this->total;
	}
