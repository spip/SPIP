 ...
 *
 * @use spip_envoyer_fichier_entetes()
 * @use spip_envoyer_fichier_entier()
 * @use spip_envoyer_fichier_partie()
 *
 ...
 * @throws Exception
 */
function spip_envoyer_fichier($fichier, $content_type = 'application/octet-stream', $options = []){
	...
	spip_envoyer_fichier_entetes($fichier, $content_type, $options['attachment'] && !$options['range'], $options['expires']);

	if (!is_null($options['range'])) {
		spip_envoyer_fichier_partie($fichier, $options['range']);
	}
	else {
		spip_envoyer_fichier_entier($fichier);
	}
}
 ...
 *
 * @see spip_envoyer_fichier()
 * @param string $fichier
 * @param string $content_type
 * @param false $attachment
 * @param int|string $expires
 */
function spip_envoyer_fichier_entetes($fichier, $content_type = 'application/octet-stream', $attachment = false, $expires=0){
 ...
 * @param string $fichier
 */
function spip_envoyer_fichier_entier($fichier){
 ...
 * @throws Exception
 */
function spip_envoyer_fichier_partie($fichier, $range=null){
	if (!file_exists($fichier)){
		throw new \Exception(sprintf('File not found: %s', $fichier));
	}
