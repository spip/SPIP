...
function squelette_traduit($squelette, $sourcefile, $phpfile, $boucles)
{
    ...
    $noms = substr(implode(', ', array_keys($boucles)), 0, -2);
    if (CODE_COMMENTE) {
        $code = "
/*
...
function analyse_resultat_skel($nom, $cache, $corps, $source = '')
{
    ...
    if (stripos($corps, 'header') !== false
        ...
        foreach ($regs as $r) {
            ...
            $j = implode('-', array_map('ucwords', explode('-', strtolower($r[2]))));
...
function synthetiser_balise_dynamique($nom, $args, $file, $context_compil)
{
    ...
    $r = sprintf(CODE_INCLURE_BALISE,
        ...
        $nom,
        implode(', ', array_map('argumenter_squelette', $args)),
        implode(', ', array_map('_q', $context_compil)));
...
function calculer_where_to_string($v, $join = 'AND')
{
    ...
    if (!is_array($v)) {
        ...
        if (strtoupper($join) === 'AND') {
            return $exp.implode(" $join ", array_map('calculer_where_to_string', $v));
        } else {
