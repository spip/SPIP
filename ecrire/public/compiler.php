...
function calculer_boucle_nonrec($id_boucle, &$boucles, $trace)
{
    ...
    if (count($boucle->separateur)) {
        $code_sep = ("'".str_replace("'", "\'", implode('', $boucle->separateur))."'");
    }
...
function calculer_requete_sql($boucle)
{
    ...
    $init[] = calculer_dec('groupby',
        'array('.(($g = implode("\",\n\t\t\"", $boucle->group)) ? '"'.$g.'"' : '').')');
    $init[] = calculer_dec('select', 'array("'.implode("\",\n\t\t\"", $boucle->select).'")');
...
function calculer_liste($tableau, $descr, &$boucles, $id_boucle = '')
{
    ...
    if (_request('var_mode_affiche') != 'validation') {
        ...
        return "join('', array_map('array_shift', \$GLOBALS['debug_objets']['sequence']['$nom'] = array(".implode(" ,\n$tab",
            $codes).')))';
    }
}
