...
function spip_pg_create_index($nom, $table, $champs, $serveur = '', $requeter = true)
{
    if (!($nom or $table or $champs)) {
        spip_log("Champ manquant pour creer un index pg ($nom, $table, (".@implode(',', $champs).'))',
            'pg.'._LOG_ERREUR);
...
function spip_pg_select(
    ...
    $query = 'SELECT '.$select
        .(!$from ? '' : "\nFROM $from")
        .(!$where ? '' : ("\nWHERE ".(!is_array($where) ? calculer_pg_where($where) : (implode("\n\tAND ",
...
function spip_pg_insertq($table, $couples = array(), $desc = array(), $serveur = '', $requeter = true)
{
    ...
    return spip_pg_insert($table, '('.implode(',', array_keys($couples)).')', '('.implode(',', $couples).')', $desc,
        $serveur, $requeter);
}
...
function spip_pg_replace($table, $values, $desc, $serveur = '', $requeter = true)
{
    ...
    if (!$where) {
        return spip_pg_insert($table, '('.implode(',', array_keys($values)).')', '('.implode(',', $values).')', $desc,
            $serveur);
    }
