...
function spip_mysql_insertq($table, $couples = array(), $desc = array(), $serveur = '', $requeter = true)
{
    ...
    return spip_mysql_insert($table, '('.implode(',', array_keys($couples)).')', '('.implode(',', $couples).')', $desc,
        $serveur, $requeter);
}
...
function spip_mysql_insertq_multi($table, $tab_couples = array(), $desc = array(), $serveur = '', $requeter = true)
{
    ...
    $cles = '('.implode(',', array_keys(reset($tab_couples))).')';
    $valeurs = array();
    $r = false;
    ...
    foreach ($tab_couples as $couples) {
        ...
        if (count($valeurs) >= 100) {
            $r = spip_mysql_insert($table, $cles, implode(', ', $valeurs), $desc, $serveur, $requeter);
            $valeurs = array();
        }
    }
    if (count($valeurs)) {
...
{
    return spip_mysql_query("REPLACE $table (".implode(',', array_keys($couples)).') VALUES ('.implode(',',
...
{
    $cles = '('.implode(',', array_keys($tab_couples[0])).')';
    $valeurs = array();
    foreach ($tab_couples as $couples) {
        $valeurs[] = '('.implode(',', array_map('_q', $couples)).')';
    }
...
{
    if (is_array($valeurs)) {
        $valeurs = implode(',', array_map('_q', $valeurs));
    } elseif ($valeurs[0] === ',') {
