...
function article_modifier($id_article, $set=null)
{
	...
	$t = Sql::getfetsel("statut", "spip_articles", "id_article=".intval($id_article));
...
function article_inserer($id_rubrique)
{
	...
	} else $row = Sql::fetsel("lang, id_secteur", "spip_rubriques", "id_rubrique=$id_rubrique");
	...
	$id_article = Sql::insertq("spip_articles", $champs);
...
function article_instituer($id_article, $c, $calcul_rub=true)
{
	...
	$row = Sql::fetsel("statut, date, id_rubrique", "spip_articles", "id_article=$id_article");
	$id_rubrique = $row['id_rubrique'];
	...
	AND (Sql::fetsel('1', "spip_rubriques", "id_rubrique=$id_parent")))
	{
...
function editer_article_heritage($id_article, $id_rubrique, $statut, $champs, $cond=true)
{
	...
	if (isset($champs['id_rubrique'])) {
		...
		if (Sql::fetsel('1', 'spip_articles', "id_article=$id_article AND langue_choisie<>'oui' AND lang<>" . Sql::quote($langue))) {
			$champs['lang'] = $langue;
		}
	}
	...
	Sql::updateq('spip_articles', $champs, "id_article=$id_article");
