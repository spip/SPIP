...
function objet_dupliquer_liens($objet, $id_source, $id_cible, $types=null, $exclure_types=null)
{
	...
	foreach($tables as $table_sql => $infos) {
		if ((is_null($types) OR in_array($infos['type'],$types))
		AND (is_null($exclure_types) OR !in_array($infos['type'],$exclure_types)))
		{
			if (objet_associable($infos['type'])) {
				...
				foreach($liens as $lien) {
					$n++;
					if ($infos['type']==$objet) {
						objet_associer(array($objet=>$id_cible),
							array($lien['objet']=>$lien[$lien['objet']]), $lien);
					}
					else {
						objet_associer(array($infos['type']=>$lien[$infos['type']]),
							array($objet=>$id_cible), $lien);
					}
				}
			}
		}
	...
	foreach ($objets_source as $objet=>$ids) {
		if ($a = objet_associable($objet)) {
			...
			elseif (reset($ids)=="NOT") {
				...
				$ids = Sql::allfetsel($primary,$l,$where);
	...
	foreach ($objets as $objet => $id_objets) {
		...
		foreach ($id_objets as $id_objet) {
			...
			AND !Sql::getfetsel(
					$primary,
					$table_lien,
					array('id_objet='.intval($id_objet), 'objet='.Sql::quote($objet), $primary.'='.intval($id))))
			{
					$e = Sql::insertq($table_lien, array('id_objet' => $id_objet, 'objet'=>$objet, $primary=>$id));
					if ($e!==false) {
						$ins++;
...
function lien_where($primary, $id_source, $objet, $id_objet)
{
	...
	$where = array();
	if ($id_source!=='*')
		$where[] = (is_array($id_source)?Sql::in(addslashes($primary),array_map('intval',$id_source),$not):addslashes($primary) . ($not?"<>":"=") . intval($id_source));
	elseif ($not)
	...
	if ($objet!=='*')
		$where[] = "objet=".Sql::quote($objet);
	if ($id_objet!=='*')
		$where[] = (is_array($id_objet)?Sql::in('id_objet',array_map('intval',$id_objet),$not):"id_objet" . ($not?"<>":"=") . intval($id_objet));
	elseif ($not)
		$where[] = "0=1"; // idiot mais quand meme
...
function lien_delete($objet_source,$primary,$table_lien,$id,$objets)
{
	...
	foreach($objets as $objet => $id_objets) {
		...
		foreach($id_objets as $id_objet) {
			...
			$liens = Sql::allfetsel("$primary,id_objet,objet",$table_lien,$where);
...
function lien_optimise($objet_source, $primary, $table_lien, $id, $objets)
{
	...
	foreach ($objets as $objet => $id_objets) {
		...
		foreach ($id_objets as $id_objet) {
			...
			$r = Sql::select("DISTINCT objet",$table_lien,$where);
			while ($t = Sql::fetch($r)) {
				$type = $t['objet'];
				$spip_table_objet = table_objet_sql($type);
				$id_table_objet = id_table_objet($type);
				$res = Sql::select("L.$primary AS id,L.id_objet",
					...
					// il faut les eliminier en repetant la condition dans le where L.objet='xxx'
								"$table_lien AS L
									LEFT JOIN $spip_table_objet AS O
										ON (O.$id_table_objet=L.id_objet AND L.objet=".Sql::quote($type).")",
						"L.objet=".Sql::quote($type)." AND O.$id_table_objet IS NULL");
				// sur une cle primaire composee, pas d'autres solutions que de virer un a un
				while ($row = Sql::fetch($res)) {
					$e = Sql::delete($table_lien, array("$primary=".$row['id'],'id_objet='.$row['id_objet'],'objet='.Sql::quote($type)));
					if ($e!=false) {
						$dels+=$e;
			...
			$res = Sql::select("L.$primary AS id",
...
function lien_set($objet_source, $primary, $table_lien, $id, $objets, $qualif)
{
	...
	foreach ($objets as $objet => $id_objets) {
		...
		foreach ($id_objets as $id_objet) {
			$where = lien_where($primary, $id, $objet, $id_objet);
			$e = Sql::updateq($table_lien,$qualif,$where);
			if ($e===false)
				$echec = true;
			else
				$ok++;
...
function lien_find($objet_source, $primary, $table_lien, $id, $objets)
{
	...
	foreach ($objets as $objet => $id_objets) {
		...
		$liens = Sql::allfetsel('*', $table_lien, $where);
...
function lien_propage_date_modif($objet, $ids)
{
	...
	AND isset($desc['field']['date_modif']))
	{
		...
		$where = (is_array($ids)?Sql::in($primary, array_map('intval',$ids)):"$primary=".intval($ids));
		Sql::updateq($table, array('date_modif'=>date('Y-m-d H:i:s')), $where);
	}
}
?>
