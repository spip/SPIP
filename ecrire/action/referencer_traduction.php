...
function action_referencer_traduction_dist($objet, $id_objet, $id_trad)
{
	...
	if ($id_trad) {
		...
		$id_lier = Sql::getfetsel('id_trad', $table_objet_sql, "$id_table_objet=".intval($id_trad)." AND NOT($id_table_objet=".intval($id_objet).")");
		if ($id_lier === NULL) {
		...
		// objets
		if ($id_lier == 0) {
			Sql::updateq($table_objet_sql, array("id_trad" => $id_trad), "$id_table_objet IN ($id_trad, $id_objet)");
		}
		// si id_lier = id_objet alors on veut changer la reference de tout le groupe de trad
		elseif ($id_lier == $id_objet) {
			Sql::updateq($table_objet_sql, array("id_trad" => $id_trad), "id_trad = $id_lier");
		}
		// sinon ajouter notre objet dans le groupe
		else {
			Sql::updateq($table_objet_sql, array("id_trad" => $id_lier), "$id_table_objet=".intval($id_objet));
		}
	}
	// on a fourni un id_trad nul : sortir id_objet du groupe de trad
	else {
		$old_id_trad = Sql::getfetsel('id_trad',$table_objet_sql,"$id_table_objet=".intval($id_objet));
	  // supprimer le lien de traduction
		Sql::updateq($table_objet_sql, array("id_trad" => 0), "$id_table_objet=".intval($id_objet));

		// Verifier si l'ancien groupe ne comporte plus qu'un seul objet. Alors mettre a zero.
		$cpt = Sql::countsel($table_objet_sql, "id_trad=".intval($old_id_trad));
		if ($cpt == 1)
			Sql::updateq($table_objet_sql, array("id_trad" => 0), "id_trad=".intval($old_id_trad));
	}
